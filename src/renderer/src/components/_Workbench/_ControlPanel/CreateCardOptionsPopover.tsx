...
export default function CreateCardOptionsPopover({
	...
	const setPopoverRef = useCallback(
		(node: HTMLDivElement | null) => {
			...
			function handleKeyDown(event: globalThis.KeyboardEvent): void {
				const buttons = Array.from(
					popover.querySelectorAll<HTMLButtonElement>("[data-card-option]")
				)
				if (buttons.length === 0) return
				const activeIndex = buttons.findIndex((button) => button === document.activeElement)
				const currentIndex = activeIndex === -1 ? selectedIndexRef.current : activeIndex
		...
		[autoFocusFirst, onClose, selectIndex]
	)
