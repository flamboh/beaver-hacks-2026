	...
	return (
		...
		>
			...
			{cards.map((card) => {
				...
				const thread = findAgentThread(card, snapshot.threads)
				const isRunning = isThreadRunning(thread)
				const needsAttention = thread !== null && thread.messages.length > 0 && !isRunning
				const markerSize = needsAttention ? MARKER_SIZE + 6 : MARKER_SIZE
				...
				return (
					...
					>
						<div
							className={`absolute flex items-center justify-center rounded-full bg-neutral-950/95 shadow-sm transition-[border-color,box-shadow,color,width,height] duration-200 ${
								needsAttention
									? "border-2 border-white/70 text-white shadow-[0_0_0_3px_rgba(255,255,255,0.12),0_0_22px_rgba(255,255,255,0.65),0_0_42px_rgba(154,167,255,0.95)]"
									: "border border-white/15 text-neutral-300 shadow-black/50"
							}`}
							style={{
								left: cardW / 2,
								top: cardH / 2,
						...
						>
							<MapCardIcon card={card} active={needsAttention} />
						</div>
					</div>
				)
			})}
...
function MapCardIcon({ card, active }: { card: ControlPanelCard; active: boolean }) {
	if (card.kind === "tool") {
	...
	return (
		<svg role="img" aria-label="OpenAI" viewBox="0 0 24 24" className="h-2.5 w-2.5">
			<path
				...
				fill={active ? "#DDE3FF" : "#9aa7ff"}
			/>
		</svg>
	)
}

function findAgentThread(
	card: ControlPanelCard,
	threads: ReturnType<typeof useAgentSnapshot>["threads"]
): ReturnType<typeof useAgentSnapshot>["threads"][number] | null {
	if (card.kind === "tool") return null
	const threadId = card.thread_id ?? `thread:${card.id}`
	return threads.find((agentThread) => agentThread.id === threadId) ?? null
}

function isThreadRunning(
	thread: ReturnType<typeof useAgentSnapshot>["threads"][number] | null
): boolean {
